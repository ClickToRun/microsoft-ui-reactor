...
| `REACTOR_OPT_001` | Reactor.Controlled | Info | ✔ | `OptionalSentinelAnalyzer` | §4.2 | Member ∈ `{SelectedIndex, SelectedPageIndex, Date, Time}` assigned sentinel `-1`/`null` in initializer/`with`; one `GetTypeInfo` confirms `Optional<T>`. Two fixes: `Optional<T>.Unset` / `Optional.Of(x)`. |
