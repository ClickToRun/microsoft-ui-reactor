...
`src/Reactor.Wrappers.Generator` is an `IIncrementalGenerator` (netstandard2.0, Roslyn 4.8, mirrors `Reactor.Analyzers`). It injects a marker attribute and, for each annotated partial element record (`[GenerateReactorWrapper(typeof(T))]`), fills the rest of that same partial — element props, descriptor, Pattern-A registration, and a parameterized factory — implementing the resolved decisions above.
...
- **IVT attribute collision** — **NOT A PROBLEM (P5.3).** Feared: the generator emits `internal` marker attributes into `Reactor.dll`, which via `InternalsVisibleTo` to `Reactor.Tests`/`Reactor.AppTests.Host` (which also run the generator) could ambiguate (CS0433). Empirically, C# resolves a type defined in the *current* assembly over one imported via IVT, so consumers build clean.
