...
`ElementPool.CleanElement` clears far more attached properties than
instance ones, so this was the larger half of `REACTOR_POOL_001`'s
subject matter — all of it in the "your write is silently discarded"
class. The failure is identical either way, so both shapes report the
*same* diagnostic id rather than a new one; only the matching differs.
