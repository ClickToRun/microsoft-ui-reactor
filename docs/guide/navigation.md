...
| `Frame(...)` | element | Raw WinUI Frame for XAML-page interop; `.Navigating`, `.Navigated`, `.NavigationFailed`. Works with code-only `Page` subclasses — Reactor publishes the target to the XAML metadata chain. |
...
The target does **not** need a `.xaml` file. WinUI resolves a navigation target
through the application's XAML metadata, which normally only covers pages the
XAML compiler saw; Reactor publishes the type you pass to `Frame(...)` into that
metadata itself, so a plain `class MyPage : Page` written entirely in C# works.

Because Reactor activates the page reflectively, `Frame(...)`'s `sourcePageType`
carries `[DynamicallyAccessedMembers(PublicParameterlessConstructor)]`. Passing a
`typeof(MyPage)` literal always satisfies it. If you keep the page type in a
variable, a field, or hook state, carry the same annotation there — otherwise a
trimmed or NativeAOT publish reports `IL2077` and could drop the constructor:

```csharp
[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicParameterlessConstructor)]
Type _current = typeof(HomePage);
```

If you navigate a `Frame` imperatively instead — `Frame().Set(f => f.Navigate(typeof(MyPage)))` —
...
managed exception you can break on.
