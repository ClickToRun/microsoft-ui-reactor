...
public sealed class ComponentHierarchySample : GallerySample
{
    ...
    {
        "Button" => Button(node.Name, null).FontSize(11).Padding(10, 4, 10, 4),
        "ToggleSwitch" => ToggleSwitch(false, null, "On", "Off").MinWidth(0).FontSize(10),
        "Slider" => Slider(50, 0, 100).Width(90).Height(32),
        "TextBox" => TextBox("", placeholderText: node.Name).Width(90).FontSize(11).Padding(6, 4, 6, 4),
        "CheckBox" => CheckBox(false, label: node.Name).FontSize(10).MinWidth(0),
        _ => HeaderBadge(node.Name),
    };
