...
| `REACTOR_ITEMS_002` | warning | An `ItemsView(...)` `viewBuilder` lambda returns something whose static type can never be an `ItemContainerElement` (e.g. `(p, i) => Border(...)`) | Wrap the returned element: `(p, i) => ItemContainer(Border(...))`. ItemsView's inner `ItemsRepeater` requires an `ItemContainer` root — without it the framework throws at mount time. Ships a code fix. Silent when the return type is only known as `Element`, is a type parameter, or is a conditional with mixed branch types. |
