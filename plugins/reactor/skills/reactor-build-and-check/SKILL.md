...
| `REACTOR_LIFECYCLE_001` | warning | Event subscription via `.Set(c => c.Event += h)` (re-subscribes every render) | Use `.OnMountAdd(c => ((TControl)c).Event += h).OnUnmountAdd(c => ((TControl)c).Event -= h)`. Keep `h` a stable delegate (static method or field). |
