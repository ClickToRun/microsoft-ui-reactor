...
| `REACTOR_LIFECYCLE_002` | warning | `UseEffect(() => …)` allocates a timer / `IObservable` subscription / event with no cleanup | Return a cleanup from the effect (picks the `Func<Action>` overload): `UseEffect(() => { var t = new PeriodicTimer(…); …; return () => t.Dispose(); }, …);`. The `Action` overload can't tear down, so the producer fires after unmount into a dead context. |
