...
| `REACTOR_HOOKS_003` | warning | `UseEffect(async () => …)` compiles as **async void** — exceptions escape the flush pipeline, cleanup decouples from the await, the setter can fire after unmount | Move the awaited work into a local `async Task RunAsync(CancellationToken ct)` and start it from a sync effect: `UseEffect(() => { var cts = new CancellationTokenSource(); _ = RunAsync(cts.Token); return () => cts.Cancel(); }, deps);`. |
| `REACTOR_HOOKS_010` | warning | Reference state (`List`/array/class) mutated in place, then the **same reference** re-passed to its setter — the setter compares by reference and skips the re-render | Pass a new value: `setItems([.. items, item])`. Never `setItems(prev => …)` — the setter is `Action<T>`, not a functional updater. |
