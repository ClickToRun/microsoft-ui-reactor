...
and paste the table the failure prints. (`-p:SkipSignaturesGen=true` keeps the build from also regenerating `skills/reactor.api.txt`, so a ledger-only edit leaves nothing else modified in your tree.) Hand-incrementing was how the published counts became unreproducible (issue #959): two branches editing `37 → 38` from the same base produce identical edits that git auto-resolves as agreement, silently dropping one increment.
