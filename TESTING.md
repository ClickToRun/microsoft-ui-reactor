...
| `Run aborted after fixture 'X'` / `Run aborted before fixture 'X'` | The Host process died mid-run (crash / `FailFast`) around `X`. | Check the tail output and stderr for the native fault. |
...
| `REACTOR_SELFTEST_HANG_TIMEOUT_SECONDS` | 60 | Host off-dispatcher hang watchdog (disabled under a debugger). |
