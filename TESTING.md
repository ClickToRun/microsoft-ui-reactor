...
| Same prefixes, with ` (Host finished its run, then exited abnormally)` | The Host reached the end of its run — the trailer is present — and then failed to exit cleanly. | Look at teardown, not at `X`. Issue #680 is one known cause. |
